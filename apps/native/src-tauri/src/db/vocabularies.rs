...
impl LocalDatabase {
    ...
    pub fn search_vocabularies(
        ...
        let sql = if let Some(_lang) = language {
            "SELECT id
             FROM vocabularies
             WHERE word LIKE ?1 AND language = ?2 AND deleted = 0
             ORDER BY word
             LIMIT 50"
        } else {
            "SELECT id
             FROM vocabularies
             WHERE word LIKE ?1 AND deleted = 0
             ORDER BY word
             LIMIT 50"
        };
    ...
    /// Delete a vocabulary (soft-delete for sync)
    ...
    /// Import a vocabulary with a specific ID (used for sync/restore operations)
    pub fn import_vocabulary_with_id(&self, vocab: &Vocabulary) -> SqlResult<()> {
        let id = vocab.id.as_deref().ok_or_else(|| {
            rusqlite::Error::InvalidParameterName("Vocabulary must have an id for import".to_string())
        })?;
        let now = Utc::now().timestamp();
        ...
        for (index, definition) in vocab.definitions.iter().enumerate() {
            ...
            tx.execute(
                "INSERT INTO vocabulary_definitions
                 (id, vocabulary_id, meaning, translation, example, order_index, created_at)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
                params![def_id, id, definition.meaning, definition.translation, definition.example, index as i32, now],
            )?;
        }
