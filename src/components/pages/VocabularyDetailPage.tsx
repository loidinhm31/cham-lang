    ...
    <>
      ...
      <div className="px-4 pt-6 space-y-6">
        ...
        <Card variant="gradient">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <span className={`${levelColors[vocabulary.level]} text-white text-sm font-bold px-4 py-2 rounded-full`}>
          ...
          <div className="space-y-4">
            {vocabulary.definitions.map((def, idx) => (
              <div key={idx} className="p-4 bg-white/40 rounded-2xl">
                <p className="text-lg font-semibold text-gray-800 mb-2">{def.meaning}</p>
                {def.translation && (
                  <p className="text-base text-teal-700 mb-2">📝 {def.translation}</p>
                )}
                {def.example && (
        ...
        {vocabulary.example_sentences.length > 0 && (
          <Card variant="glass">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('vocabulary.exampleSentences')}</h2>
        ...
        {vocabulary.topics.length > 0 && (
          <Card variant="glass">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('vocabulary.topics')}</h2>
        ...
        {vocabulary.related_words.length > 0 && (
          <Card variant="glass">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">{t('vocabulary.relatedWords')}</h2>
            <div className="space-y-3">
              {vocabulary.related_words.map((related, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 bg-white/40 rounded-xl">
                  <span className="font-semibold text-gray-800">{related.word}</span>
