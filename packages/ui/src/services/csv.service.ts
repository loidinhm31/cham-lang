...
export class CsvService {
  ...
  static async importVocabularies(
    ...
    const result = await service.importVocabulariesCSV({
      ...
      create_missing_collections: request.create_missing_collections,
    });
  ...
  static async importSimpleVocabularies(
    ...
    const parsed = this.parseSimpleVocabularyText(request.csv_text);
    if (parsed.length === 0) {
      return {
        success: false,
        rows_imported: 0,
    ...
    if (hasCollectionNames && !request.target_collection_id) {
      ...
      for (const w of parsed) {
        ...
        csvLines.push(
          [
            ...
            escape(w.word),
            escape(w.definition),
            escape(request.default_language || "en"),
          ].join(","),
        );
      }
